...
class VideoDatasetMultiClips(VideoDataset):
	...
	def __loading(self, path, video_frame_indices):
		...
		for clips_frame_indices in video_frame_indices:  # 对每帧图片分别进行处理
			...
			segments.append(
				[min(clips_frame_indices), max(clips_frame_indices) + 1])  # segments 也是一个列表，每一项都是每个视频的帧数目两个端点
	...
	def __gititem__(self, index):
		...
		if isinstance(self.target_type, list):
			target = [self.data[index][t] for t in
			          self.target_type]  # 若为列表（['video_id', 'segment']），则让target = 视频名和端点的组合
