...
def get_opt():
	...
	if opt.root_path is not None:
		...
		if opt.resume_path is not None:
			opt.result_path = opt.root_path / opt.resume_path
	...
	if opt.pretrain_path is not None:
		...
		opt.n_classes = opt.n_pretrain_class
...
def get_train_utils(opt, model_parameters):
	...
	if opt.train_crop == 'random':
		spatial_transform.append(
			RandomResizedCrop(
				...
				(opt.train_crop_min_ratio, 1.0 / opt.train_crop_min_ratio)))  # 初始化空间数扩增
	...
	spatial_transform = Compose(spatial_transform)  # 数据初始化
